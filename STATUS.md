...
> ### 🔨 [ux-audit-fixes](docs/plans/doing/ux-audit-fixes/plan.md) — Phases 1–4 DONE, Phase 5 partial (2/7). Carl to walk / decide the rest.
> The recovered 2026-07-14 UX audit → a 5-phase plan. **P1 return path ✅ green-lit.** **P2 right doors/roles ✅ green-lit.** **P3 one language ✅ self-signed** (Carl "go to the end") — one "1:1" noun everywhere, human stage names, discard dialog, member voice, VOICE.md. **P4 returns measurable ✅ self-signed** — `report-returns.ts` (verified against the real local DB); M10 already-built; derived from existing tables, no new migration. **P5 craft batch 🔨 2/7** — M8 clickable Team cards (verified live) + M11 invite sheet done; **M5 top-bar, M6 accent budget, M12 change-password (recommend split — security endpoint), M15 mobile wrap, X1 star reframe NOT built.** Offline throughout: suite 149/149, typecheck clean, both apps build; no paid runs; all local on `main` (`7d5db937` + earlier), unpushed. **▶ Your move:** walk P3/P4/P5-so-far when you like; tell me to finish P5's 5 items (and whether to build or split M12). Live board: https://claude.ai/code/artifact/b5f23732-4629-4350-86ae-b3ea662541a8
...
- Last updated: 2026-07-17 (ux-audit-fixes autonomous run — P1+P2 green-lit, P3 one-language + P4 returns-report self-signed (Carl "go to the end"), P5 craft batch 2/7 (M8 clickable cards verified live, M11 invite sheet); M5/M6/M12/M15/X1 remain. Suite 149/149, all local `7d5db937`)
