...
> **P2 ⬜ next** — the server-side half: no waiting screen is ever left un-told (terminal guarantee, per-screen isolation, `abortStage`). Includes a **guaranteed** hang found en route: "Suggest different topics" drops the work without telling anyone waiting. P1 hands the manager a Retry; **P2 makes sure that Retry lands somewhere sound** — so it follows closely. P3 ⬜ (render-crash → error, the one hang the watchdog can't catch).
...
> **▶ Your move:** nothing on P1 — it's closed. Say the word to start P2. Live board: https://claude.ai/code/artifact/91fb3ca5-9597-4d48-9b98-e5c1059366ba

> ### 🔨 [ux-audit-fixes](docs/plans/doing/ux-audit-fixes/plan.md) — ALL 5 PHASES BUILT (M12 split out). Awaiting Carl's walk + the M12 call.
> The recovered 2026-07-14 UX audit → a 5-phase plan, now complete bar one split-out item. **P1 return path ✅ green-lit.** **P2 right doors/roles ✅ green-lit.** **P3 one language ✅ self-signed** — one "1:1" noun everywhere, human stage names, discard dialog, member voice, VOICE.md. **P4 returns measurable ✅ self-signed** — `report-returns.ts` (verified against the real local DB); M10 already-built; derived from existing tables, no migration. **P5 craft batch ✅ 6/7 self-signed** — M5 setup counter no longer lies ("Step 1 of 2" for a known person) + top-bar overflow fixed · M6 one blue action, Delete → ⋯ · M8 clickable Team cards · M11 invite sheet (no window.prompt) · X1 "prep rating" · M15 phone rows. **M5/M6/M8 verified live.** **⏸ SPLIT OUT: M12** (account settings + change-password) — a security endpoint needing a live log-out/log-in round-trip; Carl's call to build or walk. Offline throughout: suite **150/150**, typecheck clean, both apps build; **zero paid runs**; all local on `main`, unpushed. **▶ Your move:** walk P3/P4/P5 when you like; say the word on M12. Live board: https://claude.ai/code/artifact/b5f23732-4629-4350-86ae-b3ea662541a8
...
> **▶ Your move:** the dot-meter is the one thing waiting — open a prep briefing on the customer app; if it looks right it rolls to the other 11 layouts.
