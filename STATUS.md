...
2. **The repeating coach panel is half fixed, and phase 2 is the half that matters** — you sent the screenshot ("the right panel is repeating itself"), and [coach-hints-live](docs/plans/doing/coach-hints-live/plan.md) P1 ✅ green-lit today (commit `ef4646cf`): the agenda question now carries its own coaching instead of borrowing whole-meeting prep cues. P2 is the one that stops it going stale, and it needs one ~$0.35 run to prove the model follows the new rule. Nothing needed from you until that proof exists. [Board](https://claude.ai/code/artifact/f5eac651-d4e8-4ba9-a1b7-c19835cd56c4).
...
| [coach-hints-live](docs/plans/doing/coach-hints-live/plan.md) | 2 phases fixing the Support panel you screenshotted, where the coaching beside the question repeated itself and ignored the answers. Three causes found: `q_intro_agenda_check` was the last question in the app with no hints of its own (built in code, so the static-content guard never covered it); the fallback cues it borrowed are computed once per session, so every hintless question shows identical cards; and `plan-turn.md` copies hints verbatim on carried-forward questions, so coaching written before the meeting never sees the transcript. **P1 ✅ green-lit 2026-07-31** (commit `ef4646cf`): the agenda question carries its own three lines, and a new guard walks the code-minted questions for count, both kinds, no em dash and the 20-word ceiling. P2 🔨 the prompt rule that rewrites the next question's listen-for lines against what was just said, £0 until one ~$0.35 proof run. **Spend note: ~84k tokens went on a local run because the dev server holds an OpenAI key the shell can't see; the shell check said "free" and it was not.** [Board](https://claude.ai/code/artifact/f5eac651-d4e8-4ba9-a1b7-c19835cd56c4) · [mockup](https://claude.ai/code/artifact/6c24a91b-b025-4146-8e86-ba24a624889a). |
