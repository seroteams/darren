...
## ▶ Your move
1. **Walk the brief star rating in your Feedback inbox** — [brief-star-rating](docs/plans/doing/brief-star-rating/plan.md) P2 is built and waiting on you. P1 ✅ green-lit today (commit `e21ed525`): the prep brief now asks "How good is this brief?" and takes a one-tap score out of 5. P2 puts every score in `/admin/feedback` as its own row type with a link into that 1:1. Rate a brief, then open the inbox. [Board](https://claude.ai/code/artifact/6de9e218-84d0-4ad7-b860-8998d3fc1e3c) · [mockup](https://claude.ai/code/artifact/e9d9a80f-93df-4141-a679-3ec7a4443cb2). £0 spent: the prep stream replays a cached brief, so no engine run was needed to verify it.
...
| Build | State |
|---|---|
| [brief-star-rating](docs/plans/doing/brief-star-rating/plan.md) | 2 phases. P1 ✅ green-lit 2026-07-30 (commit `e21ed525`) — "How good is this brief?" and five stars at the foot of the prep brief, saved per run, guests counted. The trap it closed: the recap verdict's upsert matched on run id alone, so a second run-tied feedback moment would have silently overwritten it; both now scope on (run id, kind), migration `0023`. P2 🔨 built, awaiting your walk (Brief rating rows in the Feedback inbox with the score and a link into the 1:1). £0 spent. [Board](https://claude.ai/code/artifact/6de9e218-84d0-4ad7-b860-8998d3fc1e3c). |
