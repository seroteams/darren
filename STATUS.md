...
> ### 🔨 [ux-audit-fixes](docs/plans/doing/ux-audit-fixes/plan.md) — Phases 1 + 2 ✅ GREEN-LIT 2026-07-17. Phase 3 next.
> The recovered 2026-07-14 UX audit became a 5-phase, 44-item plan. **Phase 1 ("the return path") ✅** (`6346f891`). **Phase 2 ("right doors, right roles") ✅ green-lit 2026-07-17** — B1 one member home per app (login + reload agree via a shared resolver, no split-brain) · M9 person page survives a refresh (both apps' boot) · M4 QA verdict page internal-only (manager's Review opens the clean run detail) · B2 render-time member guard on the prep flow · B6 confirmed already-resolved by past-1on1-view. Offline: suite 148/148 (+landing 3/3), typecheck clean, both apps build; no paid runs. Commits `4a9e4cec` (+earlier) on `main`, local. **Next: Phase 3 — One language** (noun sweep · cancel-vs-reset dialog fix · member-voiced copy · all ten C1–C10 rewrites). Phases 4–5 ⬜. Live board: https://claude.ai/code/artifact/b5f23732-4629-4350-86ae-b3ea662541a8
...
- Last updated: 2026-07-17 (ux-audit-fixes Phase 2 ✅ GREEN-LIT — right doors/right roles: one member home, person deep-links survive, QA page internal-only, member prep-flow guard; `4a9e4cec`, Phase 3 next. Phase 1 ✅ earlier same day)
