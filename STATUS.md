...
no-inference, dedup, and the closer. [planner-prompt-trim](docs/plans/doing/planner-prompt-trim/plan.md)
...
## ▶ Your move
1. **The planner's rule sheet is trimmed, and phase 2 is your call to start** — from your "should we improve the models and save money?". The measurement said the model was the wrong lever: **86% of spend is one stage**, and its rule sheet had re-bloated 40% since its last trim. [planner-prompt-trim](docs/plans/doing/planner-prompt-trim/plan.md) **P1 ✅ green-lit 2026-08-01** (commit `6d86ec58`), **£0**, no rule removed. **P2 ⬜ is the examples** (two AVOID/PREFER tables and two worked examples cut to those that earn their place) and it carries **the one paid check for this plan, ~$0.35**. Worth knowing before you start it: the whole plan lands nearer 13% than 30%, so the real prize here is a planner that follows its rules, not the money. **P3 is the one that makes it stick** — a free size check so the prompt cannot quietly grow back a third time. [Board](https://claude.ai/code/artifact/7c211d67-dc8b-43f9-8527-932cc334c6cb).
1. **The regression board can now watch the engine for you, and phase 2 is the half that costs money** — from your "lots of changes to the engine, I want to go back and see what's happening". [regression-rerun-board](docs/plans/doing/regression-rerun-board/plan.md) **P1 ✅ green-lit 2026-07-31** (commit `2365e7b0`): eight frozen test managers listed at Build → Regression, with the free safety check on top, **£0**. P2 🔨 is the paid rerun of one case (~$0.35, your click is the approval) — after that a red row tells you what changed and hands you the fix. [Board](https://claude.ai/code/artifact/89c153cc-fc8e-4b15-9b20-ee6afc141776) · [mockup](https://claude.ai/code/artifact/2f58f42b-7cd9-4b78-8732-e038337bb95f).
1. **The meeting opens on a question again, and phase 2 is one arc's exception** — you hit "How did last time's agreements go?" as the first thing in a repeat 1:1 and called it wrong-place, difficult design. It was: a form that would not let you past until every row was tapped, standing in front of the arc's own opening question, and it is the bi-weekly arc's *own* first named anti-pattern ("agenda-heavy openers that read like a HR form"). [action-review-placement](docs/plans/doing/action-review-placement/plan.md) **P1 ✅ green-lit 2026-07-31**: the actions are now offered as a quiet second button on the walk-in card you already see, "Start the meeting" goes straight to question 1, and the check-in never traps you (answer one of three and carry on; the untouched one stays open and is never given an answer). Committee record: `logs/committee/2026-07-31-action-review-placement.html`. **P2 is your call to start** — "Something feels off" should not open on an accountability ledger it may have inherited from a career conversation. [Board](https://claude.ai/code/artifact/8fa125e8-076f-4ac1-90ee-5351b4c07b70) · [mockup](https://claude.ai/code/artifact/c71c8cae-9f16-402f-889b-e48b93a109e8).
2. **The repeating coach panel is fixed and live, and P2 is the one thing here waiting on your eyes** — you sent the screenshot ("the right panel is repeating itself"). [coach-hints-live](docs/plans/doing/coach-hints-live/plan.md) P1 ✅ green-lit 2026-07-31 (`ef4646cf`); **P2 is LIVE on your "go live" but you have NOT walked it** (build `fb8737a`, 2026-08-01). Open a run and read the right panel as you answer: the lines should change every question and pick up what you just said, including on the LAST question. Proof cost $0.199, one gate run, PASS. [Board](https://claude.ai/code/artifact/f5eac651-d4e8-4ba9-a1b7-c19835cd56c4).
...
| Build | State |
|---|---|
| [planner-prompt-trim](docs/plans/doing/planner-prompt-trim/plan.md) | 3 phases, from your "should we improve the models and save money?". Measured free from logged prompts and cost files: **86% of spend is the planner stage**, it runs ~6x a session, and its rule sheet had gone from 27.6k characters on 10 July to 38.6k, a third of that growth in three days. **P1 ✅ green-lit 2026-08-01** (commit `6d86ec58`): six rules that each had four or five homes now have one, with pointers from the rest. No rule removed, 227/227, typecheck and copy lint clean, **£0**. **P2 ⬜** cuts the example tables and carries the plan's single paid check (~$0.35). **P3 ⬜** adds a free size cap so it cannot re-bloat a third time. **Stated plainly in the plan: the whole thing lands nearer 13% than 30%** — the repeats were short restatements, and most of the prompt is unique rules. A bigger cut means deleting rules, which is a separate call for you. Parked with numbers: the cache fix (~10% of a run) and the GPT-5.6 model swap (~22%, held until validation ends). Lane note: `plan-turn.md` was taken from the coach-hints-live chat on your say-so. [Board](https://claude.ai/code/artifact/7c211d67-dc8b-43f9-8527-932cc334c6cb). |
| [regression-rerun-board](docs/plans/doing/regression-rerun-board/plan.md) | 6 phases, from your "I want to go back and see what's happening after all these engine changes". Eight frozen test managers, same setup and answers every time; press Rerun and they go through today's engine, adapting to wherever the questions have moved. Free safety checks are the hard gate, one AI reviewer scores better/same/worse against last time, and you rate it yourself in the run review tool you already use. **P1 ✅ green-lit 2026-07-31** (commit `2365e7b0`): `/regression` under Build lists all 8 cases and runs the free check, **£0**. 223/223, typecheck + copy lint clean. **P2 🔨 next** — the paid rerun of one case (~$0.35, your click is the approval). Your calls on the way in: paid reruns stay **local-only** (live shows the screen, the free check and history, with the button honestly switched off), and **one** AI reviewer rather than a three-voice panel. **P6 added on your ask** — every red result opens a "What now" panel: plain-English diagnosis, what changed in the engine since that case was last good, and a copy-paste fix brief. Committee record: `logs/committee/2026-07-31-regression-rerun-board.html`. [Board](https://claude.ai/code/artifact/89c153cc-fc8e-4b15-9b20-ee6afc141776) · [mockup](https://claude.ai/code/artifact/2f58f42b-7cd9-4b78-8732-e038337bb95f). |
| [action-review-placement](docs/plans/doing/action-review-placement/plan.md) | 2 phases, from your "this is in the wrong place and a difficult design". **P1 ✅ green-lit 2026-07-31** (commit `81f6c4ef`): last time's agreed actions are offered on the walk-in card instead of served as a blocking form ahead of question 1, and the check-in's all-tapped gate is gone. 221/221, typecheck + copy lint clean, walked on the real app with seeded data, **£0**. The answer to your other question is in the code already: actions follow the **person**, not the meeting type (fenced on manager + person, no meeting type anywhere), so keep it. **P2 ⬜** the "Something feels off" exception. Two things found on the way and parked, not fixed: the same job exists twice with two stores (`session.promises[]` in the AI runner, the person-scoped `tracker_items` table behind the Monthly Check-in's "Catch-up" stage, same four chips, different field names), and on any non-question card the Support panel still shows the last question's hints. [Board](https://claude.ai/code/artifact/8fa125e8-076f-4ac1-90ee-5351b4c07b70) · [mockup](https://claude.ai/code/artifact/c71c8cae-9f16-402f-889b-e48b93a109e8). |
| [coach-hints-live](docs/plans/doing/coach-hints-live/plan.md) | 2 phases fixing the Support panel you screenshotted, where the coaching beside the question repeated itself and ignored the answers. Three causes found: `q_intro_agenda_check` was the last question in the app with no hints of its own (built in code, so the static-content guard never covered it); the fallback cues it borrowed are computed once per session, so every hintless question shows identical cards; and `plan-turn.md` copies hints verbatim on carried-forward questions, so coaching written before the meeting never sees the transcript. **P1 ✅ green-lit 2026-07-31** (commit `ef4646cf`): the agenda question carries its own three lines, and a new guard walks the code-minted questions for count, both kinds, no em dash and the 20-word ceiling. **P2 built, proved and LIVE 2026-08-01 (build `fb8737a`), NOT walked by you.** The lesson of P2 is how far the real cause sat from the plan: the prompt rule alone would have been **completely inert**, because `reconcileQueue` threw the planner's fresh coaching away one line before it reached the queue whenever a question was carried forward. Fixing that still left the **last question of every meeting** stale, because the reserved closer is stashed whole at bank time and reaches the front by three other routes, none of which passed through the fix. All four paths closed. One paid proof run, **$0.199** (gate `biweekly-priya` PASS), which is also what found the closer gap: 5 of 6 questions had fresh coaching, the 6th did not. The closer fix was then re-proved free on that run's own recorded data. 227/227, typecheck and both guards clean. **Spend note: ~$0.15 also went unapproved on a local run, because the dev server holds an OpenAI key the shell cannot see and the shell check said "free".** [Board](https://claude.ai/code/artifact/f5eac651-d4e8-4ba9-a1b7-c19835cd56c4) · [mockup](https://claude.ai/code/artifact/6c24a91b-b025-4146-8e86-ba24a624889a). |
