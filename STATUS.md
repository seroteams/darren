...
> ### 🔨 [ux-audit-fixes](docs/plans/doing/ux-audit-fixes/plan.md) — Phase 2 BUILT, awaiting Carl's walk. Phase 1 ✅.
> The recovered 2026-07-14 UX audit became a 5-phase, 44-item plan. **Phase 1 ("the return path") ✅ green-lit 2026-07-17** (`6346f891`). **Phase 2 ("right doors, right roles") 🔨 built 2026-07-17** — B1 one member home per app (login + reload agree via a shared resolver, no split-brain) · M9 person page survives a refresh (fixed in BOTH apps' boot) · M4 QA verdict page internal-only (manager's Review opens the clean run detail) · B2 render-time member guard on the prep flow · B6 confirmed already-resolved by past-1on1-view (not removed). Offline: suite 148/148 (+landing 3/3), typecheck clean, both apps build; no paid runs. Commits `4a9e4cec` (+earlier) on `main`, local. **Next: Carl walks phase-2.md's 5 scenarios on localhost:3000 → "green light Phase 2" → phase-close, then Phase 3.** Phases 3–5 ⬜. Live board: https://claude.ai/code/artifact/b5f23732-4629-4350-86ae-b3ea662541a8
