...
> **Parked follow-ups — ✅ ALL CLEARED 2026-07-17 (Carl: "finish this shit"), deployed:** **logo** now resolves against the `/admin/` base (`import.meta.env.BASE_URL`) across login/forgot/reset — no longer works only by luck (`0f8597ff`); **login photos** had the identical base bug (silently removed to a gradient locally) — same fix, verified on screen (`f32cb242`); **error-card copy** now leads with the real message (e.g. the timeout line) instead of burying it under *Technical details* (`0f8597ff`); **stream auto-reconnect** built the safe way — a bounded 2-try reconnect that **strips `?regenerate=1`** so it can only re-attach, never loop a paid run (unit-proven), recovering transient drops silently (`f32cb242`). Suite **153/153**, typecheck clean, admin build passes.
> **▶ Your move:** nothing — track + all follow-ups closed and live. Walk it on sero.team whenever you fancy. Live board: https://claude.ai/code/artifact/91fb3ca5-9597-4d48-9b98-e5c1059366ba
...
> ### ✅ [briefing-before-during-after](docs/plans/done/briefing-before-during-after/plan.md) — TRACK CLOSED 2026-07-17 (Carl: "1 fine go"), $0, local only.
...
> ### 🔨 In flight right now — parallel sessions (refreshed by the clean-up sweep 2026-07-17)
> These are building across parallel chats. Status per each folder:
> - **[team-page-redesign](docs/plans/doing/team-page-redesign/plan.md)** — **P3 ✅ shipped LIVE 2026-07-17** (`70ab15bd`); **P2 (person `opened_at` signal) 🔨 built, awaiting your walk**. (A CSS import + Remind wiring also ride uncommitted in `team.ts` by design, to avoid sweeping another session.)
> - **[wrap-up-exit](docs/plans/doing/wrap-up-exit/plan.md)** — **P1 🔨 in progress** (single-phase; direction green-lit 2026-07-17). The manual-door alternative Carl picked over adaptive-early-close.
> - **[personal-data-security](docs/plans/doing/personal-data-security/plan.md)** — **security track.** P1 ✅ (cross-company leak fixed + key moved to header); **P2 built, not yet committed, awaiting walk**; **P3 (git-history scrub) 🟡 interim** — the full destructive history rewrite is pending an all-sessions-closed + force-push window.
> - **[members-page](docs/plans/doing/members-page/plan.md)** — P1/P2 green-lit; **P3–P5 built, awaiting your one final walk** (see the members-page box above).
> - **[promises-loop](docs/plans/doing/promises-loop/plan.md)** — **P1 ✅ GREEN-LIT 2026-07-12** (Q9 fork → confirm card → "Locked in ✓", 1 paid run ~$0.35; `47c0024b` + `6aadec58`); **⏸ dormant since 07-12 — P2 (card zero: resurface + close-out) and P3 ⬜ never started.** Half-built (capture works, resurface/close-out don't); the orphaned `outcomeCheck` consumer is still unwired. **Not archive-safe — un-park to finish or formally park.**
> - **[admin-live-deploy](docs/plans/doing/admin-live-deploy/plan.md)** — P1 ✅ GREEN-LIT 2026-07-12. **P2/P3 built and now MERGED to `main` (`ffc5165d`, 2026-07-14) — awaiting your walk** (the old "merge blocked / on work/admin-serve" note is stale; the branch is gone). P4–P6 ⬜. **Not archive-safe.**
> - **design-stage-native** — ⏸ **PARKED to [future/](docs/plans/future/design-stage-native/plan.md)**. Dormant since 07-10; P1 built but never QA-walked, P5 blocked on your parity sign-off. Not archive-safe — un-park to finish.
