...
| 2 | Card zero — resurface + close-out | Prior promises fetched for same person; "Before question 1" card with yes/partly/no/changed taps; outcomes written back + roll-up to `outcomeCheck` | ⬜ |
...
**Next: phase 2 (card zero — resurface + close-out), another session.** Not started.
