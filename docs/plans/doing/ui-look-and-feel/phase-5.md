# Phase 5 — The frame & the customer flagship
...
## Not in this phase
- Long-tail admin pockets, alerts, overlays, off-barrel CSS (Phase 6).
- Any guided-flow layout/navigation change (parked — Carl's separate call).

## Done when
- [ ] `npm test` + `npm run typecheck` green.
- [ ] Seen on the running app in both themes: a full guided run end-to-end, briefing, member home,
      Team on a slow reload.
...
2. **Flagship flow** — run a monthly check-in end to end. No tiny text; buttons and chips match
   the rest of the app; works in dark mode; the flow itself behaves exactly as before.
