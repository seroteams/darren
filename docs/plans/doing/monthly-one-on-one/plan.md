...
## Current state
**Phase 1 ✅ GREEN-LIT 2026-07-13.** The internal-only "Monthly Check-in" card is live in the admin picker
and the real 7-stage runner opens, auto-saves, and resumes on reload. Details + verification in [phase-1.md](phase-1.md).
- **Now: `npm run typecheck` clean (3 configs) · `npm test` 131/131 · verified end-to-end in-browser.**
- Build runs on branch `work/monthly-checkin`. **Next: Phase 2 (trackers + side panels)** — promises/requests/goals
  that persist per person and resurface next meeting, replacing the prototype's sample "Aisha" data with real
  per-person tracker data. Starts on Carl's go.
- One accepted schema deviation: `guided_sessions.person_name` (denormalised; a parallel session had already
  created the table with it — see phase-1.md).
