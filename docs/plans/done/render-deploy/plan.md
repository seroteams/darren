...
**Parked follow-ups:** rotate the live DB password (passed through chat — Carl deferred); pre-approve `git push` + Render-API curl in `.claude/settings.json` (auto-mode blocked it; Carl approves the prompt on first `/release`); cleaner custom domain; delete the temporary `TONIGHT.md` runbook; Starter plan ($7/mo, no sleep) when demos need it; persistent disk for `logs/` run artifacts.
