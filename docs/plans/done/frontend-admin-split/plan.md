...
| 4 | Serve + fence the two apps | **Carl's pick A (2026-07-08): the public deploy serves the CUSTOMER app only; the admin app never ships (local tool).** server.ts serves `frontend/dist` in prod; render.yaml builds the customer app; the always-on bundle-fence + real-prod-boot test ([test-customer-serving.js](../../../scripts/test-customer-serving.js)) proves it on every `npm test`. [phase-4.md](phase-4.md) | ✅ |

⬜ not started · 🔨 in progress · ✅ done (tested)

## Current state
**✅ TRACK CLOSED 2026-07-08 — all phases (1 · 2 · 2b · 3 · 4) green-lit by Carl. $0 total spend.**
The split is complete and enforced: the customer app is its own built app; the admin app carries no
customer shell; the customer bundle carries no internal-tool code or keys (checked on every
`npm test` by [test-customer-serving.js](../../../scripts/test-customer-serving.js), which also
boots a real production server and proves the public root serves the customer app); the Render
blueprint builds and serves the customer app only — the admin console never leaves Carl's machine.

### Parked follow-ups (from this track)
- **login.js member landing → Past 1:1s** + drop MEMBER_HOME from admin — one-liner, blocked on
  guest-run's in-flight login.js edit (see phase-3.md).
- **Option B hosting** (admin app at an internal path behind login) — only if admin-on-the-go is
  ever wanted.
- **Workspaces (decision A revisit)** — plain-folder sharing held up fine; revisit only if the
  customer app grows its own dependency set.

**Phase 3 ✅ GREEN-LIT by Carl 2026-07-08 — detail:**
