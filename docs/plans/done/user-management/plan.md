...
> **✅ TRACK CLOSED — ALPHA-COMPLETE 2026-07-09.** Phases 1–4 done (table + change-role +
> deactivate/reactivate + delete), covering everything needed to manage alpha testers. Phase 5
> (reset-password/invite) is **parked by Carl's call** — lowest value now, highest risk (a public
> no-login endpoint), best built when real users need self-service recovery. Walks were waived this
> session (his standing call); all phases verified by tests + a real local-DB check, `npm test` 109/109.
> Un-park Phase 5 as a fresh Darren-Method phase when the time comes.
...
| 5 | Reset password / invite | `POST …/reset-password`; single-use hashed expiring token → copyable link | 🅿️ parked (2026-07-09, Carl's call) — lowest value now, highest risk (public endpoint); build when real users need self-service recovery + a security check |
...
- **Frontend ✅ built + committed (`65e6ae04`):** the `⋯` menu now has **Deactivate** (confirm first — signs them out now) / **Reactivate**; a switched-off user shows a **"Deactivated"** tag + a dimmed, struck-through row. `shared/api.js` gained `deactivateUser`/`reactivateUser`; a 409 guardrail surfaces the plain server message. Admin typecheck exit 0, admin build ✓, `npm test` **65/65**.
