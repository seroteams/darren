...
- Offline proof: `npm test` 167/167 (auth file now 32 cases), `npm run typecheck` clean; both apps build.
- **✅ CLOSED — Carl confirmed 2026-07-21** ("call it closed, crack on with the corridor test"). Whole 15 Jul audit is done: 20/22 fixed in the original pass, 2 tails cleaned up (B4 heading, M2 labels), account settings finished (edit-your-name + manager-only company rename), 1 tail parked (member run-detail — degrades gracefully). All committed local; ships on the next "go live".
