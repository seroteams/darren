...
**✅ TRACK CLOSED 2026-08-02. Both phases built and green-lit, a few pence total.** A repeat 1:1
opens on the arc's own question again, last time's actions are offered rather than served, the
check-in never traps anyone, and the one arc that must not open on a ledger meets them at the
recap instead. Carl walked P2's three scenarios on his own account (`a`, 2026-08-02).

Answered on the way, from the code rather than from opinion: **actions follow the person, not the
meeting type** — and that fact is exactly why the feels-off exception exists.
