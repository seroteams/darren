...
| | |
|---|---|
| Runs flagged | **28 of 78**, 44 turns (corrected twice on 2026-08-02, see below) |
| Runs clean | 50 |
...
wind-down). **Then corrected again in Phase 5**, because exempting that window outright
silenced a real gap: the snag genuinely never got asked about. The gate now judges in-window
snags against the next question and late snags against the closer. The honest split is **30
in-window misses and 14 late-snag misses**, where the 24 / 39 first reported here conflated
the two and the 19 / 30 second reported here hid the second kind entirely. See
