...
- **Relational-arc gate:** Bi-weekly check-in and "Something feels off" draw only from `wellbeing`/`topic` entries — never a `competency` entry (those read as a hidden performance review). Enforced in the prompt and by `runFocusArcGate` (`backend/engine/golden-checks.ts`), surfaced as the `FOCUS_ARC_LEAK` hard-fail in the trust gate.
