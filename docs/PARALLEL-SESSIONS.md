...
**The problem.** Several Claude sessions in the *same* folder (`Sero/seroengine`) share one git
...
From the main copy (`Sero/seroengine`):
...
- folder `../seroengine-error-log`
- branch `work/error-log`
- with `.env` copied and `node_modules` linked (so tests/build/dev run right away).

Then start the Claude session with that folder as its working directory (`cd ../seroengine-error-log`).
