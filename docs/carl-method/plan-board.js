#!/usr/bin/env node
/*
 * plan-board.js — the "Where we are" board (part 3 of the Carl Method).
 *
 * Reads a plan folder and draws it as a single visual board the product owner can open:
 ...
 *
 * Status is DERIVED from the phase files, never flagged by hand:
 *   "## ✅ GREEN-LIT" header → signed · "## Built" section → built · neither → not started
 *
...
// ===== CONFIG — edit these three things for your project ===================
// 1. Name shown in the eyebrow, top-left of the board.
const PROJECT = 'YOUR PROJECT';

// 2. Where plan folders live, relative to the repo root.
const PLANS_DOING = 'docs/plans/doing';
const PLANS_DONE = 'docs/plans/done';

// 3. The one-time "how do I start the app" panel. Plain HTML; delete the
//    lines array entirely if the project has nothing to set up.
const SETUP = {
  eyebrow: 'BEFORE YOU TEST — ONE SETUP',
  lines: [
    'Start the app (<code>npm run dev</code>), then:',
    '<code>localhost:3000</code> — the app',
    'Sign in with the <b>test account</b> on the login screen.',
  ],
};
// ===========================================================================
...
const dir = path.resolve(PLANS_DOING, slug);
const planPath = path.join(dir, 'plan.md');
if (!fs.existsSync(planPath)) {
  const alt = path.resolve(PLANS_DONE, slug, 'plan.md');
  if (fs.existsSync(alt)) die(`"${slug}" is under ${PLANS_DONE} — boards are for live plans only.`);
...
<div class="board">
  <div class="eyebrow"><span class="dot">●</span> ${esc(PROJECT)} · ${esc(eyebrowTitle)}</div>
  ...
  <div class="foot">Auto-generated from ${esc(PLANS_DOING)}/${esc(slug)}/ · re-run scripts/plan-board.js to refresh</div>
</div>`;
