...
- **Phase 004 (Backend API v1) is DONE & signed off.** The whole backend is clean layers —
  ...
  (`backend/api/services/<domain>/<domain>.repo.ts`). Archived at `docs/archive/done/backend-api-v1/`.
...
## Key files
- **This plan:** `docs/archive/done/postgres-foundation/plan.md` (+ `phase-1.md`; phase-2/3/4 to be written).
...
> `docs/archive/done/postgres-foundation/handover.md`, then PLAN.md + phase-1.md, then tell me plainly where we
