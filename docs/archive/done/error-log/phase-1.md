...
- **A small co-located writer** (mirrors [superadmin-audit.ts](../../../../backend/api/middleware/superadmin-audit.ts)): a pure `errorLogEntry(...)` builder — which **stamps the `environment`** (local vs live, from the env signal locked in Phase 0) and redacts secrets — plus an injectable sink `appendErrorLog(entry)`, so the logic is unit-tested without touching the database.
...
- [ ] The row is tagged with the right `environment` — one made on local reads `local`; the hosted path reads `production`.
...
1. **A real error gets recorded, tagged Local** — trigger a known failure (e.g. open a run whose log is missing, or the dev "force error" path). I show you the new database row: your name/email, the route, the message, the time — and it's tagged `local`, since you made it on your machine. You should recognise the error. ❌ Not OK if nothing was written, or it's tagged wrong.
