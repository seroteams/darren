// Thin controller for the Error log (error-log Phase 2 + 3). `list` is enforced at the
// route (superadminV1 in server.ts), so it already runs as the allowlisted superadmin —
// no per-handler guard. `report` is the app's own write path (POST /api/v1/errors), gated
// only by origin + rate-limit at the route; it records a client-side error best-effort.
