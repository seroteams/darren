...
const ICON = {
  ...
  registered: icon(`<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/><path d="M19 3v4"/><path d="M21 5h-4"/>`),
...
export function createAppNav({ setState, resetSession } = {}) {
  ...
  function render({ stage, user } = {}) {
    ...
    // A superadmin-only row (Registered, PG7) shows only to Carl's account. The flag is
    // cosmetic — the real wall is the backend 403 — so hiding it here is UX, not security.
    ...
    el.querySelectorAll(".app-nav__link[data-key]").forEach((b) => {
      ...
      b.hidden = b.dataset[wanted] !== "1" || (b.dataset.superadmin === "1" && !isSuperadmin);
    });
