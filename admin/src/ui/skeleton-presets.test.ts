...
const PARITY: { preset: string; src: string; classes: string[] }[] = [
  ...
  {
    preset: "lex-rows",
    src: "admin/src/stages/lexicon-review.js",
    classes: ["lex-row", "lex-row--pick", "lex-row__pick", "lex-row__num", "lex-row__body", "lex-row__phrase", "lex-row__context", "lex-row__actions"],
  },
];
...
test("every class the presets emit is covered by a parity row", () => {
  ...
  const own =
    /^(sk|sk-leaf|sk-fill|sk-line|sk-input|sk-answer|sk-icon|sk-btn|sk-table|sk-two-col|skeleton|skeleton__card|skeleton__bar|skeleton__bar--\w+|sr-only|card|block|btn|btn--sm|l-stack|l-stack--\d+|space-y-\d+)$/;
