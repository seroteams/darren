...
function buildMessages({ scenario, arcMeta, coverage, prepBrief, transcript, evaluation }) {
  ...
  const system = [
    ...
    "`flags` surface concerns for the reader. Minor nits should NOT by themselves drop the score, and do not penalize the mere absence of a deliverable a conversation type does not require.",
    "BUT trust and honesty failures are NOT nits — score them down (2 or below): if the manager's PRIVATE notes/worries appear in employee-facing output, or the briefing diagnoses the person confidently from thin or skipped answers, that is a substantive failure. Add a flag naming it.",
