...
async function main() {
  const child = spawn(process.execPath, [path.join(ROOT, "backend", "api", "server.ts")], {
    cwd: ROOT,
    // APP_ENV: "local" — this test only proves prod STATIC serving (base paths, 401 shape),
    // not live-DB behaviour. Without it, NODE_ENV=production classifies the boot as "live"
    // and server.ts refuses to start with no DATABASE_URL (fine locally via .env, red in CI).
