...
- `reason` — two parts: an observation from the notes (or the honest fallback `"No specific signal — selected because this is a normal <meeting_type> topic for a <seniority> <role>"`), then a tentative interpretation or exploration intent using language like "could be", "one possibility", "worth clarifying whether", "might be ___ or ___ or just ___".
...
- When a note doesn't speak to a point, use `"No specific signal — selected because this is a normal <meeting_type> topic for a <seniority> <role>.."` Short honesty beats fabricated detail.
</epistemic_rules>
...
- Performance & feedback / Growth & career plan → the meeting type drives; notes are supporting context. Lean on `competency` entries suited to the level.
...
- **Fallback budget.** When the manager's notes contain a concrete observation (anything other than "n/a", "fine", "nothing flagged", or empty), no more than **2 of 5** reasons may use the "No specific signal" fallback. If you find yourself reaching for a third fallback, replace it with a note-adjacent probe via the underlying dimension.
</proportioning>
...
4. Fallback honesty:
When there is no note-based signal, use this format:
"No specific signal — selected because this is a normal <meeting_type> topic for a <seniority> <role>."
...
**Example 4** — demonstrates Mid-level competency anchoring + probing a behavioural note via the underlying dimension (not the assessment)
(Mid Web Designer / Growth & career plan; notes: "Has been very negative in team meetings recently"):

- `feedback` — "How critique conversations have been landing — both directions." — "Notes mention negativity in team meetings. Could be unspoken frustration about how feedback is given or received — worth opening the door before guessing which."
- `quality` — "Where you want your design craft to stretch this year." — "No specific signal — at Mid in a Growth & career plan, craft direction is a default anchor."
- `recognition` — "Wins from the last quarter that may not have landed publicly." — "Negativity in meetings sometimes correlates with feeling unseen. Don't decode it — open the door to what they're proud of and see what surfaces."
- `collaboration` — "How design fits into product and engineering conversations right now." — "Web Designer collaborates daily with PM and frontend. If the meetings are where the negativity shows, this is where the friction may live."
- `growth` — "What 'next level' looks like in concrete design moves." — "Standard Growth & career plan anchor; for a Mid IC this is where direction is set."

Note for the generator: the negativity is named in `reason` fields as an underlying dimension to probe; it is never named in `label` text. Two fallbacks; three note-engaged points; every label is visibly design-flavoured.
