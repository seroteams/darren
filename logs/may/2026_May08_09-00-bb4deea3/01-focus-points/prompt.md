...
2. Specificity:
At least 3 focus points must visibly reflect the role, seniority, meeting type, or manager notes.
If the same list could fit a different role with only the name changed, it is too generic.
...
```
He has not taken holiday an dfeels like he is burning out
```
