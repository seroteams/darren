...
// ── audit small sweep: the numbers say what they are out of ──────────────────────
// A bare "-5" in red told a sighted reader nothing. The track always carried
// aria-valuemin/max, so only the screen was missing the scale.

test("a scale legend sits under the bars once anything has been read", () => {
  const html = renderAxisMemory([[{ id: "clarity", score: -5, read_status: "read" }]]);
  assert.match(html, /axis-mem__scale/, "the legend renders");
  assert.match(html, /-6 to \+6/, "it names the range");
  assert.match(html, /concern/, "and which direction is bad");
});

test("the number carries the scale on hover too", () => {
  const html = renderAxisMemory([[{ id: "clarity", score: -5, read_status: "read" }]]);
  assert.match(html, /title="-5 on a scale of -6 to \+6"/);
});

test("no legend above a block where nothing was ever read", () => {
  // renderAxisMemory returns "" in that case, so there is nothing to caption.
  assert.equal(renderAxisMemory([[{ id: "clarity", score: null, read_status: "not_read" }]]), "");
});
