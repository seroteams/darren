...
const field = (label: string, inner: string): string =>
  `<div class="mcr-field"><label>${esc(label)}</label>${inner}</div>`;
...
const historyBlock = (history: TrackerEvent[]): string =>
  history.length
    ? `<div class="mcr-hist">${history.map((h) => `<div>${eventLine(h)}</div>`).join("")}</div>`
    : `<div class="mcr-hist"><div>No history yet.</div></div>`;
...
export function panelHtml(panel: Panel, trackers: GroupedTrackers, copy: CopyCtx): string {
  ...
  if (panel.type === "request") {
    ...
    body = `
      ...
      <div class="mcr-panel__meta">${r.category ? `<span class="mcr-row__cat">${esc(CATEGORY_LABELS[r.category] ?? r.category)}</span>` : ""}<span class="mcr-status mcr-status--${statusClass(r.status)}">${esc(STATUS_LABELS[r.status] ?? r.status)}</span></div>
    ...
    body = `
      ...
      <div class="mcr-panel__meta"><span class="mcr-row__pct">${g.progress}%</span><span class="mcr-status mcr-status--${statusClass(g.status)}">${esc(STATUS_LABELS[g.status] ?? g.status)}</span></div>
      <div class="mcr-bar"><span style="width:${g.progress}%"></span></div>
  ...
  const foot = `<button type="button" class="mcr-btn mcr-btn--outline" data-close>${panel.type.startsWith("add-") ? "Cancel" : "Close"}</button><button type="button" class="mcr-btn mcr-btn--primary" data-save>${esc(saveLabel)}</button>`;
