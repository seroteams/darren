import{E as j,S as u,G as d,M as R,H as E,f as T,p as C}from"./index-Lojh5UQc.js";import{c as $,o as q}from"./orb-BvJ65ChT.js";import{a as L}from"./reveal-CdKVaBU1.js";import{c as P}from"./session-reset-DJVGH_hT.js";import{C as A}from"./check-GyDbZBob.js";async function U(n,{store:e,setState:t}){const a=e.sessionId;n.innerHTML=`
  ...
  `;const i=n.querySelector(".thinking-host"),o=n.querySelector(".result-host");n.querySelector(".js-start-fresh").addEventListener("click",async()=>{await P(T)&&(j(),t({stage:u.START}))});const v=$("Analyzing context…");i.appendChild(v.el);const y=e.regenerateFocusPoints;y&&t({regenerateFocusPoints:!1});const S=y?"?regenerate=1":"",p=q(`/api/v1/sessions/${encodeURIComponent(a)}/focus-points/stream${S}`);p.on("thinking",r=>v.setLabel(r.label)).on("result",async r=>{await v.exit(),i.remove(),k(r)}).on("error",r=>{t({stage:u.ERROR,error:r.message||"Focus-point generation failed.",retryStage:u.FOCUS_POINTS})}).onError(()=>{t({stage:u.ERROR,error:"Lost connection while generating focus areas.",retryStage:u.FOCUS_POINTS})}).open();function k(r){var b;e.focusPoints=r.focus_points,e.preparation=null,e.preparationRunId=null;const l=new Set;o.innerHTML=`
