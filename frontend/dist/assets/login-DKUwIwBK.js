import{S as o,l as y,m as S,i as f}from"./index-CamYUUDR.js";function j(e){try{localStorage.removeItem("seroSessionId")}catch{}e({user:null,sessionId:null,stage:o.INTAKE,substage:"NAME"})}const E=["/login/pexels-alex-green-5699419.jpg","/login/pexels-cottonbro-4861338.jpg","/login/pexels-george-milton-6953779.jpg","/login/pexels-ketut-subiyanto-4623308.jpg","/login/pexels-sarah-chai-7267386.jpg"];async function w(e,{setState:c}){e.classList.add("stage--auth");const _=E[Math.floor(Math.random()*E.length)];e.innerHTML=`
  ...
  `;const h=e.querySelector(".js-form"),u=e.querySelector(".js-email"),p=e.querySelector(".js-password"),i=e.querySelector(".js-submit"),d=e.querySelector(".js-err");function g(s){d.textContent=s,d.hidden=!1}async function v(s){s.preventDefault(),d.hidden=!0;const a=u.value.trim(),r=p.value;if(!a||!r){g("Enter your email and password.");return}i.disabled=!0,i.textContent="Logging in…";try{const{user:l}=await y({email:a,password:r});let t=l;try{t=await S()}catch{}c({user:t,stage:f(t)?o.START:o.MEMBER_HOME})}catch(l){g(l.message||"Could not log in."),i.disabled=!1,i.textContent="Sign in"}}h.addEventListener("submit",v),e.querySelector(".js-to-register").addEventListener("click",()=>c({stage:o.REGISTER})),e.querySelector(".js-try-guest").addEventListener("click",()=>j(c));const n={},m=[{label:"Manager",email:n.VITE_DEV_LOGIN_MANAGER_EMAIL,password:n.VITE_DEV_LOGIN_MANAGER_PASSWORD||""},{label:"Admin",email:n.VITE_DEV_LOGIN_ADMIN_EMAIL,password:n.VITE_DEV_LOGIN_ADMIN_PASSWORD||""},{label:"Member",email:n.VITE_DEV_LOGIN_MEMBER_EMAIL,password:n.VITE_DEV_LOGIN_MEMBER_PASSWORD||""}].filter(s=>s.email);if(m.length>0){let s=function(r){const l=m[r];u.value=l.email,p.value=l.password,a.innerHTML="Dev login: "+m.map((t,b)=>b===r?`<strong>${t.label}</strong>`:`<button type="button" class="link js-swap" data-i="${b}">${t.label}</button>`).join(" · "),a.querySelectorAll(".js-swap").forEach(t=>t.addEventListener("click",()=>s(Number(t.dataset.i))))};const a=document.createElement("p");a.className="text-ink-dim text-sm js-dev-swap",e.querySelector(".js-to-register").closest("p").before(a),s(0),requestAnimationFrame(()=>i.focus({preventScroll:!0}))}else requestAnimationFrame(()=>u.focus({preventScroll:!0}))}function A(e){e==null||e.classList.remove("stage--auth")}export{E as LOGIN_PHOTOS,w as mount,A as unmount};
//# sourceMappingURL=login-DKUwIwBK.js.map
