import{F as j,S as u,H as d,N as R,I as T,f as C,p as E}from"./index-CamYUUDR.js";import{c as $,o as q}from"./orb-BvJ65ChT.js";import{a as L}from"./reveal-CdKVaBU1.js";import{c as F}from"./session-reset-Bf8TMvyi.js";import{C as P}from"./check-GyDbZBob.js";async function U(n,{store:e,setState:t}){const a=e.sessionId;n.innerHTML=`
  ...
  `;const i=n.querySelector(".thinking-host"),o=n.querySelector(".result-host");n.querySelector(".js-start-fresh").addEventListener("click",async()=>{await F(C)&&(j(),t({stage:u.START}))});const v=$("Analyzing context…");i.appendChild(v.el);const y=e.regenerateFocusPoints;y&&t({regenerateFocusPoints:!1});const S=y?"?regenerate=1":"",p=q(`/api/v1/sessions/${encodeURIComponent(a)}/focus-points/stream${S}`);p.on("thinking",r=>v.setLabel(r.label)).on("result",async r=>{await v.exit(),i.remove(),k(r)}).on("error",r=>{t({stage:u.ERROR,error:r.message||"Focus-point generation failed.",retryStage:u.FOCUS_POINTS})}).onError(()=>{t({stage:u.ERROR,error:"Lost connection while generating focus areas.",retryStage:u.FOCUS_POINTS})}).open();function k(r){var b;e.focusPoints=r.focus_points,e.preparation=null,e.preparationRunId=null;const l=new Set;o.innerHTML=`
          ...
          <div class="js-fp-wrapper">
            <button type="button" class="focus-point focus-point--selectable js-fp-toggle" data-fp-id="${d(s.id)}" aria-pressed="false" title="${d(s.reason||"")}">
              ...
              <div class="focus-point__body">
                ...
                ${s.reason?`<div class="focus-point__reason">${d(A(s.reason))}</div>`:""}
                ${I(s)}
              </div>
...
//# sourceMappingURL=focus-points-Bi-Prjxw.js.map
