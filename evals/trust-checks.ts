...
// Leak/state detection primitives moved to the engine leaf so the LIVE path
// (reviewer.evaluate) and this gate run the exact same code (H4). Re-exported
// below to keep this module's public API unchanged.
import {
  contentWords,
  employeeFacingText,
  managerFacingText,
...
} from "../backend/engine/serve-checks.ts";
...
// clever heuristics. The leak/state primitives + checkPrivateNoteLeak /
// checkInferredStateLeak now live in backend/engine/serve-checks.ts (imported
// above) so the live serve path runs the same code; reworded/subtle state reads
// still fall to the judge Warn.
