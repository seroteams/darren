...
**🔨 ACTIVE: [validation-kit](docs/plans/doing/validation-kit/plan.md) — the build that proves managers come back (VALIDATION STAGE's corridor-test kit).** Everything needed to run the corridor test cleanly: see who came back unprompted, let testers succeed without Carl in the room, capture a one-tap verdict on every briefing. **P1 ✅ green-lit 2026-07-10** (/tasks shows every active plan as a live ⬜/🔨/✅ per-phase checklist read from disk). Next: **P2** return-signal in User management · **P3** briefing feedback tap · **P4** first-run guidance · **P5** phone + copy pass. No nudge features anywhere — the pass bar is an *unprompted* return. Live per-phase state in [STATUS.md](STATUS.md).
