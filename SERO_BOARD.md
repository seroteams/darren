...
**✅ CLOSED 2026-08-02: [planner-prompt-trim](docs/plans/done/planner-prompt-trim/plan.md) — the planner's rule sheet says each rule once, and now has a cap that stops it growing back (2 phases green-lit, 1 skipped, £0, no paid run ever needed).** From Carl's "should we improve the models and save money?" after OpenAI's price-cut email. **Measuring first reframed the question:** 86% of spend sits in one stage, the planner that re-plans the queue after each answer, because it runs about six times a session while every other stage runs once. Per-turn prompt size dominates; the model tier barely moves it. That stage's rule sheet had been trimmed on 10 July from 46k to 28k characters and had grown back to 38.6k, **+40%, a third of it in the final three days**, as unrelated feature work each bolted on a rule. Nobody was careless; there was simply no cap. P1 gave six rules that each had four or five homes a single home each (wind-down, the agency-after-a-snag trigger, the shallow gate, no-inference, dedup, the closer), **removing no rule** — the reason being that a model reading the same rule five times follows it less reliably, not the token count. P3 is what makes it hold: `npm run lint:prompt-size`, pure Node, no deps, fails past 34,400 characters with 337 of headroom, so a wording fix passes and a whole new rule does not; raising the cap is explicitly its own deliberate commit. **Recorded honestly rather than glossed: P1 saved 3.1%, not the larger number the plan implied**, because the repeats were short restatements while the mass of the file is unique rules. **Two cost levers parked with numbers, both bigger than the trim:** the cache fix (~1,700 tokens of unchanging context billed at full rate every turn instead of the cached rate, ~10% of a run, needs a change to the shared `ai-client` message shape) and the **GPT-5.6 model swap** (~22%: Terra replaces gpt-5.4 at 20% less, Luna replaces gpt-5.4-mini at 73% less) — **held until VALIDATION STAGE ends**, because swapping the engine mid-validation muddies the one signal being measured. Model ids in the OpenAI email are marketing names; the real API ids are unconfirmed.
...
else", usability "really good" — with nine findings logged in
[docs/validation/machar-2026-07-29.md](docs/validation/machar-2026-07-29.md); the sharpest:
the recap page carried a better question than the meeting did (F1). Machar will sit with managers
2 and 3 himself on recorded calls and is lining up more testers for next week. The facilitated
session does **not** count toward the pass bar (2 of 3 return *unprompted*); the watch now is
whether he comes back on his own to prep a real 1:1. Carl's remaining move: flip Render to paid.

**✅ CLOSED 2026-07-26: [onboarding-firstrun](docs/plans/done/onboarding-firstrun/plan.md) — a brand-new manager's first minute now shows the work instead of asking for it (all 4 phases green-lit in two days, $0, engine never touched).** From Carl's "the onboarding really needs a deep dive, we cannot assume new people know Sero is a tool for 1:1s". The committee ran with **Rory Sutherland** guesting ("don't read the recipe at the door, show the cake"); Carl picked Direction A of three mocks and added his walkthrough video. What shipped: a first screen carrying the seeded example's **real** prep brief (quoted verbatim from the fixture, drift-tested, labelled a sample), the video click-to-load in YouTube's privacy host (CSP opened for that one host only, pinned by test), one button, and a **quiet rail** that holds only What is Sero? / Send feedback / Log out until the first real 1:1 exists. The research pass also caught a **live bug**: since demo-seeding began on 22 July the prep wizard had counted the example as a real 1:1, so **no new signup had ever seen its beginner guidance** — Home, the wizard and the rail now share one rule (`hasRealRuns`). Hiding the rail outright was rejected on evidence: it would have taken the customer app's only Log out with it. A post-close adversarial review the same day caught **three defects in already-green-lit work** and fixed them: an internal-account fence that keyed on the persona bench (which is off on live, so it passed locally and would have failed on the deploy), a first-visit answer that survived a logout (a veteran signing in after a first-timer would have got the newcomer's stripped rail), and a failed runs fetch that left a manager on an error card with no route to Team or Past 1:1s. 191/191, both linters, real-screen proof per phase. **LIVE 2026-07-26** on Carl's "go live" (build `1db2d5c`, sero.team, deep health db: up). The plan's final live walk — registering a fresh account on the deploy — is now the only thing left on it. Parked: an extra boot request to remove the rail's settling beat (Carl's call), and the prep brief still isn't shown on a finished run's detail view. **⚠️ The quiet rail was REVERSED on 2026-07-28** — see [empty-states](docs/plans/done/empty-states/plan.md): Carl signed in on a fresh account and asked why the app had no features, because hiding the rows made an empty app and a broken app look identical. Every row now shows from day one and each destination carries its own empty state instead. Not yet pushed, so live still hides them until the next "go live".

**▶ OPEN NOW (2026-07-27, after the full-project clean-up) — seven tracks in `docs/plans/doing/`. Everything built is committed locally and ships on the next "go live" (live build is still `1db2d5c`).**
- **[audit-fixes-jul-25](docs/plans/doing/audit-fixes-jul-25/plan.md)** — 9 phases from the 25 Jul full-app Playwright audit (256 page loads, 4 roles, 963 buttons). P1 ✅ green-lit; P2 ✅ closed unwalked 2026-07-27; P3 (the `/runs/:id` refresh dead end) is the biggest user-visible one left.
- **[entry-redesign](docs/plans/doing/entry-redesign/plan.md)** — P1 ✅ green-lit, Carl picked Version A. P2 ⬜ builds it into the real screens; its blocker (the google-signin lane on `login.js`/`register.js`) is gone now that track closed. **Nothing blocks this — it is the readiest thing to pick up.**
- **[component-consolidation](docs/plans/doing/component-consolidation/plan.md)** — one owner module per UI part. P1-P3 + P8 ✅ green-lit; **P4 ⏸️ paused by Carl 2026-07-27**; P5-P7 ⬜ were lane-blocked, and the skeleton lane has now cleared, so P4 can resume.
- **[demo-member](docs/plans/doing/demo-member/plan.md)** P2 ⬜ · **screen-gallery** ❌ **deleted 2026-07-29 on Carl's word** (feature, exports and plan folder all removed) · **[promises-loop](docs/plans/doing/promises-loop/plan.md)** P3 engine feed still to build · **[sero-run-memory](docs/plans/doing/sero-run-memory/plan.md)** P1 ✅ closed unwalked, P2-P4 ⬜.
- **Three phases closed WITHOUT a Carl walk on 2026-07-27**, on his instruction ("no more walk throughs please. just go"): audit-fixes P2, screen-gallery P2, sero-run-memory P1. Each is present in code, tested, green on the free suite — and none was seen on screen by him. Each plan says so in writing.
- **Carl's move stays the corridor test.** ✅ **The three names are locked 2026-07-29: Kate, Machar, Nora.** Machar has run his first session ([log](docs/validation/machar-2026-07-29.md)); Kate and Nora have not been asked yet. Left with Carl: ask those two, and flip Render to paid so the site never sleeps on a tester.
...
**✅ CLOSED 2026-07-30: [brief-star-rating](docs/plans/done/brief-star-rating/plan.md) — the prep brief can now tell us whether it landed (both phases green-lit in one sitting, £0, from Carl's "I want to add a simple star rating for the brief").** Sero has been writing a prep brief for every 1:1 and learning nothing about whether it was any good: the manager read it, tapped "Start 1:1 questions", and the signal was gone. That is the cheapest possible read on the product's core output, and it was the one we were not taking. The brief now carries one question and five stars, one tap, no form, guests counted; every score reaches the Feedback inbox as its own row type with a link into the meeting it came from. **The find worth keeping is structural:** `upsertVerdict` matched on `run_id` alone with no discriminator, because "one row per run" was true only while there was exactly one run-tied feedback moment. Adding a second writer to that key would have silently overwritten the recap verdict, or been overwritten by it, and a validation signal quietly going missing is a bug nobody reports. A `kind` column plus scoping both upserts on the pair cost four lines and migration `0023`; the same shape appeared in the UI, where a layout switch would have wiped a score already given. Verified end to end at zero cost by noticing the prep stream replays a cached brief, so the row was read back out of Postgres without an OpenAI call. 216/216, both linters, real-screen proof on laptop and phone. Local commits, **ships next push** — it collects nothing from real managers until then. **Directly serves VALIDATION STAGE:** the pass bar is an unprompted return, and this is the first per-artefact read on whether the thing they would return for is actually good.

**✅ CLOSED 2026-07-30: [question-support-hints](docs/plans/done/question-support-hints/plan.md) — every question in a meeting now carries its own coaching, and the question bank was found dead (all 3 phases green-lit in one sitting, from Carl's "the right support should change based on the questions").** The Support panel's whole-meeting prep-brief fallback was showing on every question. Chasing it uncovered a nine-day outage: coach-panel P2 added `hints` to the bank request's `properties` but not its `required` list, OpenAI's strict structured outputs reject that outright, and `generateBankWithFallback` swallowed the 400 into the 8 static `_seed` questions. **Three live meetings, two of them Machar's validation sessions, ran on generic stock questions with only a `console.warn`.** Fixed and proved with a real call (400 before, 8 personalised questions after), plus a whole-schema regression guard on both the bank and the planner so the next added field cannot repeat it. Then the feature itself: the bank prompt governs hint craft; the planner writes hints for every question it queues mid-meeting; hand-written coaching for all 12 intro questions, 8 seeds and 22 openers (no model cost, ever); the agenda carry-forward carries its own; a thread-follow inherits the parent's and the panel says so. Three more silent field-drops closed (the reconcile rebuild, `pickOpener`, the YAML codec, which had been writing `hints: [object Object]`). Proof: a full meeting walked live — 5 questions, 5 with their own coaching, the fallback never appeared. 214/214, local commits, ships next push. **Parked, from Carl at sign-off: the coaching only ever sees this meeting's notes. Feeding it the person's history is its own plan.**
