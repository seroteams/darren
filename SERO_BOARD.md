...
**🔨 ACTIVE: [validation-kit](docs/plans/doing/validation-kit/plan.md) — the build that proves managers come back (VALIDATION STAGE's corridor-test kit).** Everything needed to run the corridor test cleanly: see who came back unprompted, let testers succeed without Carl in the room, capture a one-tap verdict on every briefing. **P1–P3b ✅ closed 2026-07-10** (/tasks live per-phase checklist; User management answers "did they come back?" per manager; every live briefing asks "Would you run this 1:1 differently now?" — since P3b as ONE skippable Finish modal with the star rating folded in, stored per run, typed cards in the Feedback inbox. Carl's real tap landed the same evening). **P4 ✅ closed 2026-07-11** (first-run guidance on intake — orientation card + honest notes example, gated on zero runs). Next: **P5** phone + copy pass — then the corridor test can start. No nudge features anywhere — the pass bar is an *unprompted* return. Live per-phase state in [STATUS.md](STATUS.md).
