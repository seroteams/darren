...
**🔨 ACTIVE: [validation-kit](docs/plans/doing/validation-kit/plan.md) — the build that proves managers come back (VALIDATION STAGE's corridor-test kit).** Everything needed to run the corridor test cleanly: see who came back unprompted, let testers succeed without Carl in the room, capture a one-tap verdict on every briefing. **P1–P3 ✅ closed 2026-07-10** (/tasks live per-phase checklist; User management answers "did they come back?" per manager; every live briefing now asks "Would you run this 1:1 differently now?" — one tap, stored per run, visible in the Feedback inbox. P3's walk waived by Carl — agent-verified end-to-end as a guest; no real manager tap yet). Next: **P4** first-run guidance · **P5** phone + copy pass. No nudge features anywhere — the pass bar is an *unprompted* return. Live per-phase state in [STATUS.md](STATUS.md).
