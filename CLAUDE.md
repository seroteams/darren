...
- **One source of truth per question — don't let trackers multiply and rot.** Status lives in exactly two canonical places: **[STATUS.md](STATUS.md)** (tactical, per-phase) and **[SERO_BOARD.md](SERO_BOARD.md)** (strategic). The build-board badges in [admin/src/stages/tasks.js](admin/src/stages/tasks.js) mirror per-step build status. Everything else is *subordinate and must say so*: [docs/archives/prototype-to-production/PROGRESS.md](docs/archives/prototype-to-production/PROGRESS.md) is now an **append-only decisions + lessons log**, not a status source (its phase-status table tends to drift — trust STATUS.md over it); the [how-it-works changelog](docs/sero-how-it-works.html) is a **manual** founder-facing log, refreshed at each phase close (it does **not** update itself — the old "grows on its own" note was false). When closing a phase, update STATUS + SERO_BOARD + the build badge together; append (don't restate status) in PROGRESS; refresh the changelog. The 2026-06-29 pre-007 audit found three stale trackers — that's the failure mode this rule prevents.

- **Keep the chat-history log fresh — refresh + commit it as we work.** [docs/chat-history/](docs/chat-history/README.md) is our re-readable record of every Claude Code session (an `INDEX.md` plus one readable transcript per session). It's generated by [scripts/chat-log.py](scripts/chat-log.py) from the local transcripts under `~/.claude/projects/…` — **free, reads local files only, no API**. Carl chose a documented habit over an auto-hook, so it's on me: run `python scripts/chat-log.py` and commit the refreshed log at natural session boundaries (when wrapping up a chunk of work, or whenever Carl asks to look back), and push as needed. Don't let it silently drift — a stale INDEX is a miss.
