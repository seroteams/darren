...
The reply contract lives in ONE place: the **Carl output style** ([.claude/output-styles/carl.md](.claude/output-styles/carl.md)) — read it there, don't restate it here. Fallback brief if the style ever fails to load: quiet middle, then a ~120-word final message opening with 🟢 DONE / 🟡 YOUR TURN / 🔴 STUCK; detail only on "more" or "techy".
