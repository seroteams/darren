...
- **TITLE — slim, top of every reply.** *(Per-reply format — this rule must live HERE, not in a skill: skills load per-task, not per-reply.)* The reply opens with one short line: `📌 <plain-words summary of what Carl just asked>` — his current request, not what I did. Only a ⚠️ guardrail block may sit above it. The original-task anchor now lives inside the bottom box's GOAL line (below).
- **One bottom box — every reply, no exceptions.** *(Design E "Verdict banner" — Carl's pick 2026-07-14.)* End every reply (even one-liners — Carl lands at the bottom and often doesn't scroll up) with this box. A **state banner** headlines it so Carl knows the situation before reading a word; a dedicated **✅ DONE?** line gives the sign-off verdict every time:
  ```
  ╔══════════════════════════════╗
  ║   🟡   YOUR MOVE NEEDED       ║
  ╚══════════════════════════════╝
  🧭 GOAL  <CAPS summary of the thread's opening request — pinned, unchanged all thread>
  🔵 NOW   <what I just did + current state, plain words>
  🔴 DO    <your move — one action, or A)/B)/C) if there's a real choice, or "nothing right now">
  ✅ DONE? <sign-off verdict — "Not yet — <what's left>" OR "Yes — safe to sign off">
  ──────────────────────────────
  ```
  - **Banner** = the state at a glance, using this fixed vocabulary (swap the word + colour to match reality):
    - `🟢   DONE — SAFE TO SIGN OFF` (finished + verified, Carl's green light closes it)
    - `🟡   YOUR MOVE NEEDED` (needs one thing from Carl)
    - `🔵   WORKING — NOTHING NEEDED` (I'm on it)
    - `🔴   BLOCKED — NEEDS YOUR CALL` (stuck)
  - `🧭 GOAL` = caps summary of the thread's **opening** prompt; stays the SAME across the whole thread (Carl's anchor when he's flipping between many agents). This is the old `ORIGINALLY`.
  - `🔵 NOW` = what just happened + current state, plain words. On coding tasks the short files-edited rundown sits just above the box or folds into this line. (old `DOING`.)
  - `🔴 DO` = the move. Real options when they exist, **lettered `A)/B)/C)`** so Carl can reply with one letter (letter first, then ✅ recommended / ▶️ alternative / ⏸️ park, recommended pick as A). If only one path, name it on one line; if nothing's needed, say "nothing right now". (old `YOU`.)
  - `✅ DONE?` = the sign-off verdict, **always present**. If not finished: `Not yet — <the one thing still open>`. If finished + verified: `Yes — safe to sign off`. This is Carl's #1 need — never bury done-ness inside NOW. Honours the archive-safe rule: only ever say "safe to sign off" when the work is genuinely complete.
  - ⚠️ guardrail warnings still go at the TOP of the reply, never in the box. Don't stress exact box-edge alignment — keep the banner + four labels and pad to a tidy width; the terminal is monospace so the ╔╗ borders line up.
...
- **"Check point" — one word, two jobs.** Mid-work = save (snapshot commit + STATUS refresh + chat-log); fresh session = restore (read the trackers, give the full picture — never ask what he means). The full ritual is the [`checkpoint`](.claude/skills/checkpoint/SKILL.md) skill. Any commit follows the [`safe-commit`](.claude/skills/safe-commit/SKILL.md) rules — path-scoped, never sweeping another session's work.
